...
public class MainActivity extends AppCompatActivity implements TextToSpeech.OnInitListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTextReceive = (TextView)findViewById(R.id.mTextReceive);
     ...
     //   tts.speak("안녕친구들아",TextToSpeech.QUEUE_FLUSH,null);
    ...
    private void promptSpeechInput() {
        Intent intent = new Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH); //음성인식 인텐트 생성
        intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL,
                RecognizerIntent.LANGUAGE_MODEL_FREE_FORM);
        intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE, Locale.getDefault()); // 해당 지역의 언어로 설정
        intent.putExtra(RecognizerIntent.EXTRA_PROMPT,
                getString(R.string.speech_prompt));//프롬프트에 문자열 입력
        try {
        ...
        switch (requestCode) {
            case REQ_CODE_SPEECH_INPUT: {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    if(showtext.equals("안전운전")) {
                        sendData("a");
                    }
                    else if(showtext.equals("주의")) {
                        sendData("b");
                    }
                    else if(showtext.equals("감사해요")) {
                        sendData("c");
                    }
                    else {
                        sendData(showtext);
                                ...
                                if(b == mCharDelimiter) {
                                    ...
                                    handler.post(new Runnable(){
                                        ...
                                        public void run() {
                                            ...
                                            if(Double.parseDouble(data.substring(0, 5)) > 28 && count == 0) {
                                                    count++;
                                                    tts.speak("온도가 높습니다. 환기가 필요합니다.",TextToSpeech.QUEUE_FLUSH,null);
                                                }
                                                else {
                                                    if(Double.parseDouble(data.substring(0, 5)) > 28) {
                                                        count++;
                                                    }
                                                    else {
                                                        count = 0;
